...
- Per-table **SQL Inventory** sections (from Step 1)
...
7. **Caveats** — Mock vs production data distribution differences, composite index column ordering rationale, write overhead assessment. For multi-path queries, note that the relative frequency of each path in production determines the practical impact of each index.
...
Full report format example in [examples.md](examples.md).
