...
- **TTL tables**: Automatic expiration and cleanup of cold data based on time columns, can work with CCI for hot/cold data separation. **All TTL questions (archiving, expiration, auto-add partitions) must be handled by the `polardbx-ttl20` skill. Do NOT generate TTL SQL in this skill.**
